...
public class Redconsistelttele extends OpMode {
    ...
    public void init() {
        ...
        try {
            if (PoseStore.hasSavedRed()) setRobotPose(PoseStore.redPose);
    ...
    public void loop() {
        ...
        if (g2Left && !lastG2Left) setRobotPose(new Pose(104.0, 134.0, Math.PI));
        if (g2Up   && !lastG2Up)   setRobotPose(new Pose(114.0, 3.0, 0));
        if (g2Down && !lastG2Down) setRobotPose(new Pose(10.0, 10.0, 0));
        ...
        if (currentA && !lastA) {
            setRobotPose(new Pose(2.6, 2.9, 0));
        }
        lastA = currentA;
